...
// experience のラベル名
$experienceLabelName = (isset($biz_vektor_options['experienceLabelName']))? $biz_vektor_options['experienceLabelName'] : 'experience';
// post のトップのURL
$postTopUrl = (isset($biz_vektor_options['postTopUrl']))? $biz_vektor_options['postTopUrl'] : '';
// experience のトップのURL
$experienceTopUrl = (isset($biz_vektor_options['experienceTopUrl']) && $biz_vektor_options['experienceTopUrl'])? $biz_vektor_options['experienceTopUrl'] : home_url().'/experience/';
...
if ( is_404() ){
	...
	if ( isset($_GET['post_type']) && $_GET['post_type'] == 'clinic' ){
	...
	else if ( isset($_GET['post_type']) && $_GET['post_type'] == 'experience' ){
		$post_slug = $_GET['experience_cat'];

	  $page_title = $experienceLabelName;
...
// ▼▼ 固定ページ
	...
	// マルチサイトや、experienceのトップを固定ページで作った場合にURLが変動するため
	if ( is_tax() || is_tag() || is_year() || is_month() || is_single() ) {
		$panListHtml .= '<li itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a href="' . esc_url($experienceTopUrl) . '" itemprop="url"><span itemprop="title">' . $experienceLabelName . '</span></a> &raquo; </li>';
	}
	if ( is_tax() ) {
